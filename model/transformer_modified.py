        ...
        if(self.universal):
            if(config.act):
                ...
                for l in range(self.num_layers):
                    ...
                    x += tf.cast(self.timing_signal[:, :inputs.shape[1], :], inputs.data.dtype)
                    #x += self.position_signal[:, l, :].unsqueeze(1).repeat(1,inputs.shape[1],1).type_as(inputs.data)
                    x += tf.cast(tf.tile(tf.expand_dims(self.position_signal[:, l, :], 1), [1,inputs.shape[1],1]), inputs.data.dtype)
...
class Generator(tf.keras.layers.Layer):
    ...
    def __init__(self, d_model, vocab):
        ...
        self.proj = tf.keras.layers.Dense(units = vocab, activation = None, use_bias = False)
    ...
    def forward(self, x, attn_dist=None, enc_batch_extend_vocab=None, extra_zeros=None, temp=1, beam_search=False, attn_dist_db=None):
        ...
        if(config.pointer_gen):
            ...
            vocab_dist = tf.keras.activations.softmax(vocab_dist, axis = 2)
            vocab_dist_ = alpha * vocab_dist

            #attn_dist = F.softmax(attn_dist/temp, dim=-1)
            attn_dist = tf.keras.activations.softmax(attn_dist/temp, axis = -1)
            attn_dist_ = (1 - alpha) * attn_dist     
            
            
            enc_batch_extend_vocab_ = tf.concat([tf.expand_dims(enc_batch_extend_vocab, axis = 1)]*x.size(1),1) ## extend for all seq
            ...
            if(beam_search):
                ...
                enc_batch_extend_vocab_ = tf.concat([tf.expand_dims(enc_batch_extend_vocab_[0], axis = 0)]*x.size(0),0)
...
#class Transformer(nn.Module):
    ...
    def __init__(self, vocab, decoder_number,  model_file_path=None, is_eval=False, load_optim=False):
        ...
        self.decoder_key = tf.keras.layers.Dense(decoder_number, bias=False)
