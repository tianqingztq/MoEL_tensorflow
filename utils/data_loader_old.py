...
def collate_fn(data): #data is a list of dictionaries

    def merge(sequences):
        #每一个batch是8个key的dict 每一个key对应的item都是长为batch_size的list， 
        #这里的sequences是其中一个key对应的item e.g item_info['context']
        #这个function是将list里长度不同的tensor pad为一样长
    ...
    ##program
    d["target_program"] = item_info['emotion'] #same length already
    d["program_label"] = item_info['emotion_label'] #same length already
